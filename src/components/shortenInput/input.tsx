    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: linkName || "Shortened link with Cuttr",
          text: `Check out this link: ${shortLink}`,
          url: shortLink,
        });
      } catch {
        // User dismissed
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <button
                type="button"
                onClick={handleCopy}
                className="flex-1 md:flex-initial px-6 py-4 rounded-xl bg-white/10 hover:bg-white/20 text-white font-semibold transition-colors flex items-center justify-center gap-2 border border-white/10"
              >
                {copied ? <Icon.Check size={18} className="text-emerald-400" /> : <Icon.Copy size={18} />}
                {copied ? "Copied!" : "Copy"}
              </button>
              <button
                type="button"
                onClick={handleShare}
                className="flex-1 md:flex-initial px-6 py-4 rounded-xl bg-accent text-black font-semibold hover:bg-accent2 transition-colors flex items-center justify-center gap-2 shadow-md shadow-accent/20"
              >
                <Icon.Share2 size={18} />
                <span>Share</span>
              </button>
