...
const Dashboard: React.FC = () => {
  ...
  >(() => {
    try {
      const cached = localStorage.getItem("cuttr_cached_user_links");
  ...
  const handleCopyLink = (shortLink: string, id: string) => {
    try {
  ...
  const handleShareLink = async (item: { linkName: string; shortLink: string }) => {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: item.linkName || "Short link from Cuttr",
          text: `Check this link: ${item.shortLink}`,
          url: item.shortLink,
        });
      } catch {
        // User cancelled
      ...
      handleCopyLink(item.shortLink, item.linkName);
    }
  };
              ...
              if (!querySnapshot.empty) {
                ...
                Promise.all(promises).then(() => {
                  if (querySnapshot) {
                    ...
                    setArr(urls);
                    try {
                      localStorage.setItem("cuttr_cached_user_links", JSON.stringify(urls));
                    } catch (e) {
                      console.log(e);
                    }
                  }
  ...
  return isOnline || arr.length > 0 ? (
    <div className="flex flex-col items-center mt-7 relative">
      {!isOnline && (
        <div className="w-full max-w-2xl px-5 mb-4">
          <div className="p-3 bg-amber-500/15 border border-amber-500/30 rounded-xl text-center text-amber-300 text-xs font-semibold flex items-center justify-center gap-2">
            <Icon.WifiOff size={14} />
            <span>Offline Mode — Showing your saved links cached locally</span>
          </div>
        </div>
      )}
      <div
                ...
                {!item.editUrls && !item.deleteConfirm && (
                  <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleCopyLink(item.shortLink, item.id)}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-surface-light border border-white/10 text-xs font-semibold text-text-muted hover:text-white transition-colors"
                      >
                        {copiedId === item.id ? (
                          <>
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-surface-light border border-white/10 text-text-muted hover:text-accent transition-colors"
                        title="Share on mobile"
                        aria-label="Share on mobile"
                      >
                        <Icon.Share2 size={16} />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-white/5 text-text-muted hover:text-accent transition-colors"
